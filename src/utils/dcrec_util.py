...
def build_sim_graph(user_his,user_num,item_num ,mode='train'):
    k = 4
    row = []
    col = []
    for uid, item_seq in user_his[mode].items():
    ...
    cf_graph = sp.csr_matrix(([1] * len(row), (row, col)), shape=(
        user_num + 1, item_num + 1), dtype=np.float32)
...
def build_adj_graph(user_his,user_num,item_num, mode='train'):
    ...
    adj_mat.setdiag(np.ones((item_num + 1,)))
